...
#
# files.user/files.all entries are home-relative (e.g. ".dircolors", not "/root/.dircolors")
# and get expanded into one instance per real user (every config.users.users entry with
# isNormalUser = true) in collect.nix -- nixos-files has no notion of a single "primary" user.
# Any `user`/`group` set on a files.user/files.all entry is ignored; the real per-user (or
# root, for files.all's root copy) owner is always used.
#---------------------------------------------------------------------------------------------------
{ lib, pkgs, ... }:
let
  ...
  options.files = {
    ...
    user = lib.mkOption {
      ...
      description = ''
        ...
        isNormalUser = true). target is home-relative, e.g. ".config/menus", not an absolute path.
      '';
      example = ''
    ...
    all = lib.mkOption {
      ...
      default = { };
      description = ''
        Files installed both for root (/root/<target>) and for every real user
        (config.users.users entries with isNormalUser = true, at $HOME/<target>). target is
        home-relative, not an absolute path.
      '';
    };
  };
}
